...
const useNotes = () => {
  ...
  const filteredNotes = useMemo(() => {
    return notes.filter((note) => filter === 'all' ? note.isDeleted !== true : filter === 'favorites' ? note.isFavorite && note.isDeleted !== true : filter === 'tags' ? note.tags?.length > 0 && note.isDeleted !== true : filter === 'trash' ? note.isDeleted === true : true)
  }, [notes, filter])
  const searchedNotes = useMemo(() => {
    return filteredNotes.filter((note) => note.title.toLowerCase()
